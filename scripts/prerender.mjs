#!/usr/bin/env node
// Post-build step: serves the built dist/ locally, visits every route with a
// headless browser, waits for react-helmet-async + app content to render,
...
async function main() {
  ...
  try {
    for (const { path: route } of routes) {
      if (route === '/') continue; // root index.html is already the SPA shell
